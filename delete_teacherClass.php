...
if (isset($_POST['btn_delete'])) {
    ...
    $query = "UPDATE class set delete_status = 1 where class_id='$id' ";
    $query_run = mysqli_query($connection, $query);
    if ($query_run) {
        $qquery = "UPDATE  student_class set delete_status = 1 where class_id='$id' ";
        $query_runn = mysqli_query($connection, $qquery);
        if ($query_runn) {
            $_SESSION['status'] = "Archive Successfully";
            $_SESSION['status_code'] = "success";
            header("Location: teacherclass.php");
        } else {
            echo 'not deleted';
