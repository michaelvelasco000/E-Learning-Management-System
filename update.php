...
//add quiz
    ...
    $desc = $_POST['desc'];
    $id=uniqid();
    $q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name' , '$sahi' , '$wrong','$total','$time' ,'$desc','$tag', NOW())") or die('Error 123');
    
    header("location:add_question.php?q=4&step=2&eid=$id&n=$total");
    }
    ...
    header("location:view_quiz.php?q=0");
    }
    ...
    if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) {
        ...
        {
        $sn++;
        header("location:quiz_start.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
        }
        ...
        header("location:quiz_start.php?q=result&eid=$eid");
        }
            ...
            $r3 = mysqli_query($con,"DELETE FROM questions WHERE eid='$eid' ") ;
            $r4 = mysqli_query($con,"DELETE FROM quiz WHERE eid='$eid' ") ;
            $r4 = mysqli_query($con,"DELETE FROM history WHERE eid='$eid' ") ;
            ...
            if(@$_GET['q']== 'quizre' && @$_GET['step']== 25 ) {
                ...
                $sun=$sun-$s;
                $q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
                header("location:quiz_start.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
                }
    ?>
